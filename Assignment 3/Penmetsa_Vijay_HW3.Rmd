...
total_data <- read.csv("./Credit.csv", header = TRUE, stringsAsFactors = TRUE)
```
...
lm_full <- lm(Balance ~ Income+Limit+Rating+Cards+Age+Education+Student,
              data = train_data)
summary(lm_full)
```
...
x_train_nn <- model.matrix(~Income+Limit+Rating+Cards+Age+Education+Student,
                           data = train_data)[, -1]
...
x_test_nn <- model.matrix(~Income+Limit+Rating+Cards+Age+Education+Student,
                           data = test_data)[, -1]
...
library(neuralnet)
set.seed(7)
nn_full <- neuralnet(Balance~Income+Limit+Rating+Cards+Age+Education+StudentYes, 
                 data=x_train_nn, hidden=c(4,2))
```
...
nn_full_pred <- predict(nn_full, newdata=x_test_nn, type='response')[,1]*y_max
```
...
Answer:  
lm_full MAE = 84.22484   
gam_full MAE = 51.80754   
nn_full MAE = 29.34589  

**Q4(c) [2 points]**
Which statistical model do you prefer, `lm_full` or `gam_full` or `nn_full`? Give reasons.  

Answer:  
I prefer nn_full because of the significantly lower MAE compared to gam_full and lm_full.
