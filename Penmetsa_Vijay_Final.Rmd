...
###  **Q1 [6 points]** Read in Data and Data Partition
**Q1(a) [2 points]**
Let's correctly read in the data in `NYZ_Times.csv` and name it as `total_data`. 
```{r Q1(a)}
## Write code solution to Q1(a) here
total_data <- read.csv("./NYZ_Times.csv", header=T, stringsAsFactors=T)
```


**Q1(b) [2 points]** What is the conversion rate in the current dataset stored in `total_data`?
```{r Q1(b)}
## Write code solution to Q1(b) here
nrow(total_data[total_data$Subscriber == 'Yes',]) / nrow(total_data) * 100
```
  
Answer:  
Conversion rate is 9.6%.  
...
### **Q2 [12 points]** Logistic Regression
**Q2(a) [4 points]**
...
lm_full <- glm(Subscriber~Age+Experience+Income+Family+AvgAmt+Education, 
...
Answer:  
Yes. Customers with higher income are more likely to be subscribers because Income has a positive coefficient.  
...
Answer:  
Age and Experience were removed during backward selection.  
...
Answer: No. The relationship for 'Income' is non-monotonic. 


**Q3(c) [2 points]**
Keeping everything else the same, further fit a GAM mobdel but using splines with **df=2** for `Income`, name it `gam2`. Do we have a monotonic relationship for predictor `Income` now?
```{r Q3(c)}
...
Answer: Yes. The relationship for predictor 'Income' is monotonic when 'df' is changed to 2.  
...
x_train_nn <- model.matrix(~Age+Experience+Income+Family+AvgAmt+Education, 
                           data=train_data)[,-1]
...
x_test_nn <- model.matrix(~Age+Experience+Income+Family+AvgAmt+Education, 
                          data=test_data)[,-1]
...
### **Q5 [18 points]** Model Evaluation (Prediction)
**Q5(a) [4 points]**
...
Answer:  
The predicted conversion probability of the fifth customer in test data is:  
lm_full: 3.106382 %  
gam1: 0.1688447 %  
nn1: 0% (1.294744e-19 %)
...
library(caret)
lm_full_acc <- confusionMatrix(factor(ifelse(lm_full_pred>0.5, 'Yes', 'No')), test_data$Subscriber,
                               positive='Yes')

gam1_acc <- confusionMatrix(factor(ifelse(gam1_pred>0.5, 'Yes', 'No')), test_data$Subscriber,
                           positive='Yes')

nn1_acc <- confusionMatrix(factor(ifelse(nn1_pred>0.5, 'Yes', 'No')), test_data$Subscriber,
                           positive='Yes')
```
...
Answer:  
We should prefer nn1 if we have budget to only reach 20% of the target customers as it can give us close to 95% of the positive respondents as opposed to 90% from gam1.  
...
Answer:  
To capture at least 80% of the potential customers with lm_full, our marketing budget should be enough to reach out to at approximately 15% of the customers.  
